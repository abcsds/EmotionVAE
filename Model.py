...
class BetaVAE:
        ...
        for epoch in range(self.epochs):
            ...
            try:
                ...
                for i in range(self.n_batches):
                    ...
                    print("t_data {:2.4f}".format(time()-t_data), end="\t")
                    # One hot encode y
                    ...
                    _  = sess.run(self.opt,
                                         feed_dict=feed_dict)
                    print("t_opt {:2.4f}".format(time()-t_opt), end="\t")

                    t_loss = time()
                    tloss = sess.run(self.loss,
                                         feed_dict=feed_dict)
                    print("t_loss {:2.4f}".format(time()-t_loss), end="\n")
                    ...
                    if epoch == self.epochs-1:
                        ...
                        zs.append(lat)
                        ys.append(y)
                    else: # ResourceExhaustedError training time augmenting every epoch
                        del X, y
    ...
    def plot_independant(self, sess, X, y, n_subplots=5, figsize=(16, 16), scale=2):
